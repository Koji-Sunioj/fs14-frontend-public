...
const AlbumPage = () => {
  ...
  let albumName: string, artistName: string, price: number, stock: number, tags: string[]

  if (album !== null) {
    ;({ albumName, artistName, price, stock, tags } = album!)
  }
              ...
              <Card.Text>
                {tags!.map((tag) => (
                  <Button
                    size="sm"
                    variant="info"
                    style={{ margin: '2px 2px 2px 0px', pointerEvents: 'none' }}
                    key={tag}>
                    {tag}
                  </Button>
                ))}
